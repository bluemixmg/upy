...
function Header()
{
    ...
    $this->Cell(20,10,'Reporte de Disponibilidad',0,0,'C');
    $this->Ln();
...
$sql = "SELECT disponibilidad.fecha, CONCAT (disponibilidad.id_usuario, ' - ',chofer.nombre, ' ',  chofer.apellido) as cho, bloque.descripcion FROM disponibilidad 
    INNER JOIN chofer ON chofer.id_usuario = disponibilidad.id_usuario
...
if(pg_num_rows($consulta)>0){
    ...
    $pdf->MultiCell(40,10,"Fecha",0, "C", "true");
    $x= $x + 40;
    $y2 = $pdf->y_mayor($y2,$pdf->GetY());
    $pdf->SetXY($x, $y);
    $pdf->MultiCell(90,10,"Conductor",0, "C", "true");
    $x= $x + 90;
    $y2 = $pdf->y_mayor($y2,$pdf->GetY());
    $pdf->SetXY($x, $y);
    $pdf->MultiCell(60,10,"Bloque",0, "C", "true");
    $x= $x + 60;
    $y2 = $pdf->y_mayor($y2,$pdf->GetY());
    $pdf->SetXY($x, $y);
    $pdf->Ln();
    ...
    foreach ($consulta as $c){
        ...
        $pdf->MultiCell(40,10,date_format(new DateTime($c['fecha']), 'd-m-Y'),0, "C", $bandera);
        $x= $x + 40;
        $y2 = $pdf->y_mayor($y2,$pdf->GetY());
        $pdf->SetXY($x, $y);
        $pdf->MultiCell(90,10,utf8_decode($c['cho']),0, "C", $bandera);
        $x= $x + 90;
        $y2 = $pdf->y_mayor($y2,$pdf->GetY());
        $pdf->SetXY($x, $y);
        $pdf->MultiCell(60,10,utf8_decode($c['descripcion']),0, "C", $bandera);
        $x= $x + 60;
        $y2 = $pdf->y_mayor($y2,$pdf->GetY());
        $pdf->SetXY($x, $y);
