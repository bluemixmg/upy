...
function Header()
{
    ...
    $this->Cell(20,10,'Reporte de Incidencicas Por Empresa',0,0,'C');
    $this->Ln();
...
$sql = "SELECT incidencia.*,chofer.id_usuario,chofer.nombre,chofer.apellido,chofer.telefono,vehiculo.modelo,tipo_incidencia.nombre as i_nombre FROM incidencia 
    ...
    INNER JOIN cliente ON cliente.cedula = incidencia.id_cliente 
    ...
    WHERE fecha BETWEEN '".$fecha_inicio."' AND '".$fecha_fin."' AND empresa.rif='".$rif."'";
...
if(pg_num_rows($consulta)>0){
    ...
    $y2 = $pdf->y_mayor($y2,$pdf->GetY());
    $pdf->SetXY($x, $y);
    $pdf->MultiCell(50,10,"Empleado",0, "C", "true");
    $x= $x + 50;
    $y2 = $pdf->y_mayor($y2,$pdf->GetY());
    $pdf->SetXY($x, $y);
        ...
        if($c['id_cliente']==0){
            ...
            if(pg_num_rows($consulta_cliente)>0){
                foreach ($consulta_cliente as $cc){
                    $pdf->MultiCell(50,10,utf8_decode($cc['nombre']).' '.$cc['apellido'],0, "C", $bandera);
                }
            }else{
